...
class Time:
    ...
    def dayError(self):
        return (
            (self.month in [1, 3, 5, 7, 8, 10, 12] and self.day > 31)
            or (self.month in [4, 6, 9, 11] and self.day > 30)
            or (
                self.month == 2
                and (
                    (self.isSchrikkeljaar() and self.day > 29)
                    or (not self.isSchrikkeljaar() and self.day > 28)
                )
            )
        )
