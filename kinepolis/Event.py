...
class Reservation(Event):
    ...
    def update(self, eventSysem):
        eventSysem.system.getReservationSystem().reservate(self.userId, self.screeningId, self.seats)
...
class StartScreening(Event):
    ...
    def update(self, eventSystem):
        screening, succes = eventSystem.system.getScreeningSystem().retrieve(self.screeningID)
        if succes:
